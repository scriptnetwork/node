...
var ScriptJS = require('../libs/scriptjs.esm');
...
exports.updateTokenHistoryBySmartContract = async function (sc, transactionDao, accountTxDao, tokenDao, tokenSummaryDao, tokenHolderDao) {
  ...
  if (tokenName === "" && isTnt721) {
    try {
      ...
      console.log('Error in fetch SRC-721 token name by tokenURI in updateTokenHistoryBySmartContract: ', e.message);
    }
    console.log('tokenName after tokenURI function:', tokenName);
  }
  try {
    const type = 9, isEqualType = 'true', pageNum = 0, limitNumber = 0, reverse = false;
    ...
    for (let tx of txs) {
      ...
      // console.log("sc helper exp api decoded logs------->>>>>>", logs)
...
async function _getTNT20Name(address, abi) {
  ...
  try {
    ...
    const gasPrice = Script.getTransactionFee(); //feeInTFuelWei;
    const gasLimit = 2000000;
    const data = functionSignature + encodedParameters;
    const tx = Script.unsignedSmartContractTx({
      from: address,
      to: address,
      data: data,
      value: 0,
    ...
    const rawTxBytes = ScriptJS.TxSigner.serializeTx(tx);
    const callResponse = await smartContractApi.callSmartContract({ data: rawTxBytes.toString('hex').slice(2) }, { network: Script.chainId });
...
async function _getTNT721Name(address, abi) {
  ...
  try {
    ...
    outputValues = /^0x/i.test(outputValues) ? outputValues : '0x' + outputValues;
    let url = abiCoder.decode(outputTypes, outputValues)[0];
    if (/^http:\/\/(.*)api.Scriptdrop.com.*\.json(\?[-a-zA-Z0-9@:%._\\+~#&//=]*){0,1}$/g.test(url) && typeof url === "string") {
...
async function updateTokenSummary(tokenArr, address, tokenName, tokenType, abi, tokenSummaryDao, tokenHolderDao) {
  ...
  for (let key of Object.keys(holders)) {
    ...
    const tokenId = key === 'SRC20' ? null : key;
