...
var accountTxRouter = (app, accountDao, accountTxDao, accountTxSendDao, transactionDao, rpc) => {
  ...
  //             total += (tx.data.source.coins.SPAYWei - '0') / 1000000000000000000;
  //           } catch (e) {
          ...
          switch (tx.type) {
              ...
              if (data.proposer.address !== address) {
                data.outputs.forEach(output => {
                  if (output.address === address) {
                    obj.tx_type = 'Receive';
                    obj.scpt_amount = helper.formatCoin(output.coins.scptwei);
                    obj.spay_amount = helper.formatCoin(output.coins.SPAYWei);
                    obj.from = '0x00000';
                    obj.to = address;
                  }
                })
            ...
            case 2:
              if (data.inputs[0].address === address) {
                obj.tx_type = 'Send';
                obj.scpt_amount = helper.formatCoin(data.inputs[0].coins.scptwei);
                obj.spay_amount = helper.formatCoin(data.inputs[0].coins.SPAYWei);
                obj.from = address;
            ...
            case 5:
              if (data.source.address === address) {
                obj.tx_type = 'Send';
                obj.scpt_amount = helper.formatCoin(data.source.coins.scptwei);
                obj.spay_amount = helper.formatCoin(data.source.coins.SPAYWei);
                obj.from = address;
  ...
  // router.get("/totalScptWeiValue", async (req, res) => {
  //   try {
  //     let data = await accountDao.getTotalScptWeiAccountBalanceAsync();
  //     let TotalWalletBal = data[0].Total * 10e-19;
 
  //     res.status(200).send({ "totalScptWeiValue": TotalWalletBal });

  //   } catch (err) {

  //     console.log("/totalScptWeiValue", err)
  ...
  router.get("/supplyData", async (req, res) => {
    try {
      let dataSpay = await accountDao.getTotalSpayWeiAccountBalanceAsync();
      let dataScpt = await accountDao.getTotalScptWeiAccountBalanceAsync();
      let dataSpay1 = await accountDao.getTotalAccountBalanceAsync();
      let totalSpay = dataSpay[0].Total * 10e-19;
      let totalSwei = dataScpt[0].Total * 10e-19;
      res.status(200).send({ "totalSpayWeiValue": totalSpay,"totalScptWeiValue": totalSwei, totlAccountBalance: dataSpay1[0] });
