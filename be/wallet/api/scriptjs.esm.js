...
class SendTx extends Tx {
    constructor(senderAddr, outputs, feeInSPAYWei, senderSequence) {
        ...
        let feeInSPAYWeiBN = BigNumber.isBigNumber(feeInSPAYWei) ? feeInSPAYWei : (new BigNumber(feeInSPAYWei));
        for (var i = 0; i < outputs.length; i++) {
            let output = outputs[i];
            let SCPTWei = output.scptwei;
            let SPAYWei = output.spaywei;
            let SCPTWeiBN = BigNumber.isBigNumber(SCPTWei) ? SCPTWei : (new BigNumber(SCPTWei));
            let SPAYWeiBN = BigNumber.isBigNumber(SPAYWei) ? SPAYWei : (new BigNumber(SPAYWei));
        ...
        let txInput = new TxInput(senderAddr, totalSCPTWeiBN, totalSPAYWeiBN.plus(feeInSPAYWeiBN), senderSequence);
        this.inputs = [txInput];
        this.outputs = [];
    ...
    setSignature(signature) { //TODO support multiple inputs
    ...
    signBytes(chainID) {
        ...
        let ethTxWrapper = new EthereumTx(payload); // For ethereum tx compatibility, encode the tx as the payload
        let signedBytes = RLP.encode(ethTxWrapper.rlpInput()); // the signBytes conforms to the Ethereum raw tx format
        for (var j = 0; j < this.inputs.length; j++) { // Attach the original signature back to the inputs
...
class DepositStakeTx extends StakeTx {
    constructor(source, holderAddress, stakeInSCPTWei, feeInSPAYWei, purpose, senderSequence) {
        ...
        let feeInSPAYWeiBN = BigNumber.isBigNumber(feeInSPAYWei) ? feeInSPAYWei : (new BigNumber(feeInSPAYWei));
        this.fee = new Coins(new BigNumber(0), feeInSPAYWeiBN);
        let stakeInSCPTWeiBN = BigNumber.isBigNumber(stakeInSCPTWei) ? stakeInSCPTWei : (new BigNumber(stakeInSCPTWei));
        this.source = new TxInput(source, stakeInSCPTWeiBN, null, senderSequence);
        this.purpose = purpose;
        if (!holderAddress.startsWith('0x')) { //Parse out the info from the holder (summary) param
            holderAddress = "0x" + holderAddress;
        }
        if (holderAddress.length !== 42) { //Ensure correct size
            //TODO: throw error
        }
    ...
    signBytes(chainID) {
        let sig = this.source.signature; // Detach the existing signature from the source if any, so that we don't sign the signature
        ...
        let ethTxWrapper = new EthereumTx(payload); // For ethereum tx compatibility, encode the tx as the payload
        let signedBytes = RLP.encode(ethTxWrapper.rlpInput()); // the signBytes conforms to the Ethereum raw tx format
        this.source.signature = sig; // Attach the original signature back to the source
        return signedBytes;
    }
