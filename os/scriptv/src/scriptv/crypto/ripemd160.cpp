...
using c = scriptv::crypto::ripemd160;

const c::value_type c::value_type::zero_(0);
...
c::value_type c::value_type::from_b58(const string& s) {
    ...
    if (v.size() != output_size) [[unlikely]] { k.zero(); return k; }
...
bool c::value_type::set_b58(const string& s) {
    ...
    if (v.size() != output_size) [[unlikely]] { zero(); return false; }
