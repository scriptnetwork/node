...
#define logclass "sha256"
#include "logs.inc"

using namespace std;
using namespace scriptv::crypto;
using c = scriptv::crypto::sha256;
using scriptv::ko;
...
void c::transform(uint32_t* s, const unsigned char* chunk) {
...
void c::write(const vector<unsigned char>& data) {
    if (data.empty()) [[unlikely]] return;
    write(reinterpret_cast<const unsigned char*>(&data[0]), data.size());
}
...
ko c::value_type::from_b58(const string& s) {
    vector<unsigned char> v;
    if (!b58::decode(s, v)) [[unlikely]] {
        zero();
        auto r = "KO 66091";
        log(r);
        return r;
    }
    if (v.size() != output_size) [[unlikely]] {
        zero();
        auto r = "KO 66093";
        log(r);
        return r;
    }
    memcpy(data(), v.data(), output_size);
    return ok;
}

bool c::value_type::operator < (const value_type& other) const {
    static_assert(output_size == 32);
...
bool c::value_type::is_not_zero() const {
    static_assert(output_size == 32);
    for (int i = 0; i < output_size; i += 8)
        if (*reinterpret_cast<const uint64_t*>(&(*this)[i]) != 0) return true;
    return false;
}
