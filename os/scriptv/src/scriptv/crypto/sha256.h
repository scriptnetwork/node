...
namespace scriptv::crypto {
    ...
    inline ostream& operator << (ostream& os, const scriptv::crypto::sha256::value_type& v) {
        os << v.to_b58();
        return os;
    }

    inline istream& operator >> (istream& is, scriptv::crypto::sha256::value_type& v) {
        string s;
        is >> s;
        v.from_b58(s);
        return is;
...
namespace std {
  ...
  template <>
  struct hash<scriptv::crypto::sha256::value_type> {
    size_t operator()(const scriptv::crypto::sha256::value_type& k) const {
