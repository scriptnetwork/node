...
c::keys::pub_t::hash_t c::keys::pub_t::compute_hash() const {
    if (!valid) [[unlikely]] return hash_t::zero_;
    unsigned char out[ser_size];
    size_t len = ser_size;
    if (!secp256k1_ec_pubkey_serialize(ec::instance.ctx, out, &len, this, SECP256K1_EC_COMPRESSED)) [[unlikely]] {
        #if CFG_COUNTERS == 1
...
bool c::keys::pub_t::to_vector(vector<unsigned char>& out) const {
    ...
    out.resize(ser_size);
    size_t len = ser_size;
    if (!secp256k1_ec_pubkey_serialize(ec::instance.ctx, out.data(), &len, this, SECP256K1_EC_COMPRESSED)) [[unlikely]] {
        #if CFG_COUNTERS == 1
            ++counters.unable_serialize_pubkey;
        #endif
        log("KO 7060");
...
void c::keys::pub_t::read(const unsigned char* p) {
    if (!secp256k1_ec_pubkey_parse(ec::instance.ctx, this, p, ser_size)) [[unlikely]] {
        zero();
        return;
    }
    set_valid();
...
void c::keys::pub_t::read_ser(const vector<unsigned char>& ser) {
    ...
    if (!secp256k1_ec_pubkey_parse( ec::instance.ctx, this, ser.data(), ser.size())) [[unlikely]] {
        #if CFG_COUNTERS == 1
...
c::keys::priv_t c::keys::priv_t::from_hex(const string& hex) {
    priv_t k;
    int j = 0;
    istringstream is(hex);
    int isz = hex.size() / 2;
    if (isz != priv_mem_size) {
        k.zero();
        return k;
    }
    for (int i = 0; i < priv_mem_size; i += 2) {
        char x = ::tolower(hex[i]);
        unsigned char v;
...
vector<uint8_t> c::sig_encode_der(const sig_t& sig) const {
    ...
    size_t nSigLen{72};
    der.resize(nSigLen);
    if (!secp256k1_ecdsa_signature_serialize_der(ctx, (unsigned char*)der.data(), &nSigLen, &sig)) [[unlikely]] {
...
ko c::sign(const keys::priv_t& pk, const sigmsg_hasher_t::value_type& msgh, sig_t& sig) const {
    ...
    if (!secp256k1_ecdsa_sign(ctx, &sig, &msgh[0], &pk[0], secp256k1_nonce_function_rfc6979, 0)) [[unlikely]] {
        #if CFG_COUNTERS == 1
...
ko c::generate_shared_key(unsigned char* shared_key, size_t size, const keys::priv_t& priv, const keys::pub_t& pub) {
    ...
    if (!secp256k1_ecdh(ctx, temp_key, &pub, &priv[0], 0, 0)) [[unlikely]] {
        #if CFG_COUNTERS==1
...
c::keys::pub_t c::keys::get_pubkey(const priv_t& privk) {
    pub_t k;
    if (!secp256k1_ec_pubkey_create(ec::instance.ctx, &k, privk.data())) [[unlikely]] {
        #if CFG_COUNTERS == 1
...
c::keys::pub_t c::keys::priv_t::pub() const {
    pub_t k;
    if (!secp256k1_ec_pubkey_create(ec::instance.ctx, &k, data())) [[unlikely]] {
        #if CFG_COUNTERS == 1
            ++counters.error_priv_2_pub;
        #endif
        k.zero();
...
c::sig_t c::sig_from_der(const sig_der_t& der) const {
    ...
    sig_t sig;
    if (!secp256k1_ecdsa_signature_parse_der(ctx, &sig, der.data(), der.size())) [[unlikely]] {
        #if CFG_COUNTERS == 1
...
size_t std::hash<scriptv::crypto::ec::keys::pub_t>::operator() (const scriptv::crypto::ec::keys::pub_t& g) const {
    return *reinterpret_cast<const size_t*>(g.hash().data());
}

istream& scriptv::crypto::operator >> (istream& is, ec::keys::pub_t& k) {
    string s;
    is >> s;
    k.set_b58(s);
    return is;
