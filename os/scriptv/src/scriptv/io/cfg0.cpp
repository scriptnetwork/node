...
pair<ko, pair<string, string>> c::split_fqn(string fqn) {
    ...
    if (*fqn.begin() != '/') {
        auto r ="KO 50184 Not an absolute path.";
        log(r, fqn);
        return make_pair(r, make_pair("", ""));
    }
...
pair<ko, c> c::load(const string& home) {
    check_platform();
    if (!ensure_dir(home)) {
        log(KO_60534, home);
...
ko scriptv::io::write_file_(const vector<uint8_t>& v, const string& filename) {
...
ko scriptv::io::read_file_(const string& filename, vector<unsigned char>& buf) {
...
ko scriptv::io::read_file_(istream& is, vector<unsigned char>& buf) {
...
ko scriptv::io::read_text_file_(const string& filename, string& dest) {
...
string c::replace(const string& src, const string& find, const string& replace) {
    ostringstream os;
    size_t pp = 0;
    while (true) {
        auto p = src.find(find, pp);
        if (p == string::npos) break;
        os << src.substr(pp, p - pp) << replace;
        p += find.size();
        pp = p;
    }
    if (pp < src.size()) {
        os << src.substr(pp, src.size() - pp);
    }
    return os.str();
}

vector<string> c::split(const string_view& src, const string_view& sep) {
    vector<string> v;
    size_t ppos = 0;
    size_t pos = 0;
    while ((pos = src.find(sep, pos)) != std::string::npos) {
        v.emplace_back(src.substr(ppos, pos - ppos));
        pos += sep.size();
        ppos = pos;
    }
    if (ppos < src.size()) {
        v.emplace_back(src.substr(ppos, src.size() - ppos));
    }
    return v;
}
...
ko scriptv::io::system_command(const string& command, string& result) {
