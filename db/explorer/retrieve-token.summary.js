...
function main() {
  ...
  if (!config.defaultPandoChainID) {
    Logger.log('Error: unable to load config.defaultPandoChainID:', config.defaultPandoChainID);
    process.exit(1);
  }
  Pando.chainId = config.defaultPandoChainID;
  Logger.log('Pando.chainId:', Pando.chainId);
...
function setupGetBlockCronJob(mongoClient) {
  ...
  updatePreTokenSummaryCronJob.UpdateTNT20Symbol();
}
