...
exports.UpdateTNT721Name = async function () {
  ...
  try {
    tokenSummaryInfoList = await tokenSummaryDao.getRecordsAsync({ type: "SRC-721", name: "" });
  ...
  for (let i = 0; i < tokenSummaryInfoList.length; i++) {
    ...
    if (tokenName === "") {
      try {
        ...
        Logger.log('Error in fetch SRC-721 token name by tokenURI in updateTokenHistoryBySmartContract: ', e.message);
      }
...
exports.UpdateTNT20Symbol = async function () {
  let tokenSummaryInfoList = [];
  try {
    tokenSummaryInfoList = await tokenSummaryDao.getRecordsAsync({ type: "SRC-20" });
    Logger.log('tokenSummaryInfoList[0]:', tokenSummaryInfoList[0]);
    Logger.log('tokenSummaryInfoList.length:', tokenSummaryInfoList.length);
  } catch (e) {
    Logger.log('Error occurs in get tokenSummary info:', e.message);
  ...
  for (let i = 0; i < tokenSummaryInfoList.length; i++) {
    ...
    try {
      ...
      Logger.log('Error in fetch symbol in UpdateSRC20Symbol: ', e.message);
    }
...
exports.Execute = async function () {
  ...
  for (let i = 0; i < smartContractIds.length; i++) {
    ...
    if (!isTnt721 && !isTnt20) {
      Logger.log(`Smart contract #${i}/${smartContractIds.length} Add:${address} is not SRC-721 or SRC-20, skip.`);
      continue;
    }
    ...
    const tokenType = isTnt721 ? 'SRC-721' : 'SRC-20';
...
async function _getTNT20Name(address, abi) {
  ...
  try {
    ...
    const gasPrice = Pando.getTransactionFee(); //feeInTFuelWei;
    ...
    Logger.log('Error occurs in getSRC20Name:', e.message);
    return "";
  }
}
...
async function _getTNT721Name(address, abi) {
  ...
  try {
    ...
    outputValues = /^0x/i.test(outputValues) ? outputValues : '0x' + outputValues;
    let url = abiCoder.decode(outputTypes, outputValues)[0];
    if (/^http:\/\/(.*)api.Pandodrop.com.*\.json(\?[-a-zA-Z0-9@:%._\\+~#&//=]*){0,1}$/g.test(url) && typeof url === "string") {
    ...
    Logger.log('Error occurs in getSRC721Name:', e.message);
    return "";
  }
}
...
async function updateTokenSummary(tokenArr, address, tokenName, tokenType, abi, tokenSummaryDao, tokenHolderDao) {
  ...
  for (let token of tokenArr) {
    ...
    const key = token.token_id != null ? token.token_id : 'SRC20';
